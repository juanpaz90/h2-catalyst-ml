...
'''Edge gated
each neighbor j send a message to node i.
An edge gate is a learnable, multiplicative filter that modulates the message stregth per edge'''
class EdgeGatedConv(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_attr):
        ...
        gate = self.edge_gate(edge_features)
...
class hybrid(nn.Module):
    def __init__(self, hidden_dim=256, num_layers=6, num_heads=8, dropout=.15):
        ...
        for i in range(num_layers):
            if i % 3 == 0:
                ...
                layer = EdgeGatedConv(hidden_dim, hidden_dim, 128)
    ...
    def forward(self, batch):
        ...
        for i, (layer, norm, alpha) in enumerate(
            ...
            else: # apply edge gated convolutions
                x = layer(x, batch.edge_index, edge_attr)
