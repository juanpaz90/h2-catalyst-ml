...
def plot_energy_distributions(y_init, y_relaxed, deltas):
    ...
    plt.title("Distribution of Target Relaxed Energies (y_relaxed)", fontweight="bold")
    plt.xlabel("Energy (electron volts eV)", fontweight="bold")
    plt.ylabel("Frequency (System Count)", fontweight="bold")
    plt.legend()
    ...
    plt.title("Energy Change During Relaxation (y_relaxed - y_init)", fontweight="bold")
    plt.xlabel("Energy Change (eV)", fontweight="bold")
    plt.ylabel("Frequency (System Count)", fontweight="bold")
    plt.legend()
    plt.grid(axis='y', alpha=0.3)
    plt.show()
    ...
    plt.title("Correlation: Initial vs Relaxed Energy", fontweight="bold")
    plt.xlabel("Initial Energy (eV)", fontweight="bold")
    plt.ylabel("Relaxed Energy (eV)", fontweight="bold")
    plt.legend()
    plt.axis('equal')
    plt.show()
